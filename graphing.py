...
def grapher(win, people, frame):
    ...
    for i in range(frame+1):
        ...
        pygame.draw.rect(win, COLORS[0], (int(win.get_width()/frame*i), # x
                                          ...
                                          int(win.get_height()/len(people)*counter[0])))      # height

        pygame.draw.rect(win, COLORS[1], (int(win.get_width() / frame * i),  # x
                                          int(win.get_height() / len(people) * counter[0]),  # y
                                          win.get_width() / frame,  # width
                                          int(win.get_height() - win.get_height() / len(people) * counter[0])))  # height
